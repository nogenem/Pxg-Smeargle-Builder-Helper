...
async function scrapWiki(
  ...
  if (!!shouldSaveErrors) {
    ERRORS = require('../json/1-2_wikiErrors.json');
    if (!ERRORS || typeof ERRORS !== 'object') {
      throw new Error(
        '`./json/1-2_wikiErrors.json` should have an object ({})!',
      );
    }
  } else {
    ERRORS = {};
  ...
  if (!!shouldSaveJson && pokemonData.length !== prevSavedPokemonJSON.length) {
    saveJson(pokemonData, '1-1_pokemons.json');
  }

  if (!!shouldSaveErrors && Object.keys(ERRORS).length > 0) {
    saveJson(ERRORS, '1-2_wikiErrors.json');
  }
